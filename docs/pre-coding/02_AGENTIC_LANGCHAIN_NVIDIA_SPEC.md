...
Prompt 4 implementation note: section drafts now also include an explicit `intent`. The deterministic validator enforces intent-to-fact policy before rendering.
...
Prompt 4 implementation note: fallback reason codes are normalized to safe product codes such as `PROVIDER_NOT_CONFIGURED`, `PROVIDER_TIMEOUT`, `PROVIDER_RATE_LIMITED`, `PROVIDER_UNAVAILABLE`, `PROVIDER_INVALID_RESPONSE`, `GROUNDING_VALIDATION_FAILED`, and `REPAIR_FAILED`.

Transport retries remain inside the provider. Semantic repair is separate and bounded to one additional provider call after a schema-compatible but policy-invalid draft.

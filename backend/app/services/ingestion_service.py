...
from app.services.report_service import build_deterministic_report, canonical_hash
...
class IngestionService:
    ...
    def replace_clinic_day(
        ...
        source_hash = canonical_hash(
            {"clinic_id": clinic_id, "business_date": business_date.isoformat(), "records": request.records}
        )
        ...
        try:
            with self.session.begin_nested():
                clinic_day, created = self.repository.replace(
                    clinic_id=clinic_id,
        ...
        return self.repository.get(clinic_id, business_date, with_children=True), created
